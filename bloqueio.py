...
#tab-bloqueios .blq-kpis{{display:grid;grid-template-columns:repeat(5,1fr);gap:10px}}
...
<div style="padding:20px 32px">
  ...
  <div class="blq-controls">
    ...
    <select id="blq-status" onchange="blqRender()">
      ...
      <option value="inv">Em investigacao</option>
...
function blqRender() {{
  ...
  var rows = BLQ_DATA.filter(function(d) {{
    ...
    if(stF && blqGetSt(d.id)!==stF)           return false;
  ...
  if(!tbody) return;
  if(!rows.length){{
    tbody.innerHTML='<tr><td colspan="10" style="text-align:center;padding:40px;color:#374151">Nenhum driver encontrado.</td></tr>';
    return;
  }}
  ...
  tbody.innerHTML = rows.map(function(d,i){{
    ...
    if(shps.length){{
      ...
      shpRow =
        ...
        '<td colspan="10"><div class="blq-shp-meta">'+
...
def main():
    ...
    html = re.sub(
        ...
        '<span class="ver-badge">v4.4</span>',
        html, count=1
    )
